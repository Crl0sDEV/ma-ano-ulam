...
export default function Home() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(data.error || "Naku, may problema sa kusina.");
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      link.download = `Mama-${recipe.dishName
        .replace(/\s+/g, "-")
        .toLowerCase()}.png`;
      link.href = dataUrl;
      link.click();
  ...
  const toggleFavorite = (currentRecipe: Recipe) => {
    const isFavorited = favorites.some(
      (fav) => fav.dishName === currentRecipe.dishName
    );
    let newFavorites;
    if (isFavorited) {
  ...
  const isCurrentRecipeFavorited = recipe
    ? favorites.some((fav) => fav.dishName === recipe.dishName)
    : false;
    ...
    <main className="min-h-screen bg-stone-50 text-neutral-800 p-6 flex flex-col items-center font-sans relative">
      ...
      <div className="max-w-md w-full relative z-10">
        <form
          ...
          className="relative space-y-4 bg-white p-6 rounded-3xl shadow-sm border border-stone-100"
        >
          <div>
        ...
        {error && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="mt-4 p-4 bg-red-50 text-red-600 rounded-lg text-center text-sm"
          >
            {error}
          </motion.div>
        )}
              ...
              */}
              <div className="absolute top-4 right-4 z-10 flex gap-2 bg-white/80 backdrop-blur-sm p-1.5 rounded-full shadow-sm border border-stone-100 no-print">
                ...
                >
                  <Heart
                    size={18}
                    className={isCurrentRecipeFavorited ? "fill-current" : ""}
                  />
                </button>
              ...
              <div className="bg-orange-50 p-6 border-b border-orange-100 relative overflow-hidden">
                ...
                <h2 className="text-3xl font-bold text-orange-900 leading-tight font-hand mb-2 pr-28">
                  {recipe.dishName}
                </h2>
                ...
                <div className="flex items-start gap-3">
                  <button
                    onClick={handleSpeak}
                    className={clsx(
                      "shrink-0 p-2 rounded-full transition-all border no-print",
                      isSpeaking
                        ? "bg-orange-500 text-white border-orange-600 animate-pulse"
                        : "bg-orange-100 text-orange-700 border-orange-200 hover:bg-orange-200"
                    )}
                    title={isSpeaking ? "Stop Speaking" : "Listen to Mama"}
                  >
                    {isSpeaking ? (
                      <StopCircle size={18} />
                    ) : (
              ...
              <div className="p-6 space-y-8">
                <div>
                  <h3 className="font-bold text-stone-900 mb-4 uppercase text-xs tracking-wider flex items-center gap-2">
                    <Utensils size={14} className="text-orange-500" /> Mga
                    Kailangan
                  </h3>
                  <ul className="space-y-3 text-stone-600">
                    {recipe.ingredientsList.map((item, index) => (
                      <li
                        key={index}
                        className="flex items-start gap-3 p-2 rounded-lg bg-stone-50/50"
                      >
                ...
                <div className="border-t border-stone-100 pt-6">
                  <h3 className="font-bold text-stone-900 mb-4 uppercase text-xs tracking-wider">
                    Paano Lutuin
                  </h3>
                ...
                <div className="text-center text-stone-400 text-xs font-medium pt-4 border-t">
              ...
              <div className="overflow-y-auto p-4 space-y-3 flex-1">
                {favorites.length === 0 ? (
                  <div className="text-center py-10 text-stone-400">
                    ...
                    <p className="text-sm">
                      Mag-heart ka ng recipe para ma-save dito!
                    </p>
                  </div>
                ) : (
                  favorites.map((fav) => (
